...
На каждой из точек надо убедиться, что представления того, кто пишет серверный и того, кто пишет клиентский код, совпадают относительно набора полей, их имён, типов и семантики. 
...
Важно один раз обеспечить одинаковые преобразования:
- **типов** с сервера на клиент в *генераторе*
- **данных** с сервера на клиент в *сериализаторе*

— и далее можно полагаться на TypeScript в контроле контрактов. 

### Шаг 2: прокси для контроллеров - еще пример, на миллион долларов - 1

Например, в одном проекте используется запрет null внутри ядра системы, для выражения необязательности используется одна из реализаций option type — Maybe<T>. 
...
### Шаг 2: прокси для контроллеров - TypeScript уже решил проблему `null`, в отличие от C# - 1

И при отображении на клиент этот тип `Maybe<T>` преобразуется в `T | null`, а сериализатор научен разворачивать `Maybe<T>` в просто `T`, если значение есть, или в `null`, если его нет. 

Так что если у нас есть свойство "необязательная строка" на сервере в модели, как выше, то на клиенте оно отобразится как: 
